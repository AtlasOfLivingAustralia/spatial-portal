...
public class Log4jLoader {
     ...
     */
    private static String getXmlTemplateFileContents() {
        String content = null;
        try {
     ...
     */
    private static void rewriteLog4jXml(String template) {
        if (Validate.empty(template)) {
            ...
            template = template.replaceAll(SERVER_NAME_REPLACE, Config.getHostname());
            template = template.replaceAll(PORTAL_NAME_REPLACE, Config.getPortalNameStatic());
