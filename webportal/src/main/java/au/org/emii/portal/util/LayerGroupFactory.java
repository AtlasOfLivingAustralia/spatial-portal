...
public class LayerGroupFactory {
    ...
    public <T extends Facility> T createInstance(Class<T> clazz, LayerGroup xmlLayerGroup, PortalSession portalSession) {
        ...
        try {
            ...
            logger.error("unable to instantiate clazz " + clazz.getCanonicalName(), ex);
        } catch (IllegalAccessException ex) {
            logger.error("illegal access to clazz " + clazz.getCanonicalName(), ex);
        }
        ...
        // regions also need bounding box information
        if (facilityOrRegion instanceof Region) {
            // yuk!
            ((Region) facilityOrRegion).copyFrom((RegionLayerGroup) xmlLayerGroup);
        }
