...
public class SimpleRegion extends Object implements Serializable {
    ...
    private boolean isWithinPolygon_EPSG900913(double longitude, double latitude) {
        ...
        if (longitude <= bounding_box[1][0] && longitude >= bounding_box[0][0]
            ...
            //initial segment
            int longitudePx = Utils.convertLngToPixel(longitude);
            boolean segment = Utils.convertLngToPixel(points[0][0]) > longitudePx;
            ...
            for (i = 1; i < len; i++) {
                // is it in a new segment?
                if ((Utils.convertLngToPixel(points[i][0]) > longitudePx) != segment) {
                    //lat value at line crossing > target point
                    y = (int)((longitudePx - Utils.convertLngToPixel(points[i][0]))
                            * ((Utils.convertLatToPixel(points[i][1]) - Utils.convertLatToPixel(points[i - 1][1]))
                            / (double) (Utils.convertLngToPixel(points[i][0]) - Utils.convertLngToPixel(points[i - 1][0])))
                            + Utils.convertLatToPixel(points[i][1]));
                    if (y > Utils.convertLatToPixel(latitude)) {
                        score++;
                    } else if (y == Utils.convertLatToPixel(latitude)) {
                        //line crossing
                        return true;
                    }
    ...
    public int[][] getOverlapGridCells_Polygon_EPSG900913(double olongitude1, double olatitude1, double olongitude2, double olatitude2, int owidth, int oheight, byte[][] three_state_map, boolean noCellsReturned) {
        ...
        int longitude1 = Utils.convertLngToPixel(olongitude1);
        int longitude2 = Utils.convertLngToPixel(olongitude2);
        int latitude1 = Utils.convertLatToPixel(olatitude2);
        int latitude2 = Utils.convertLatToPixel(olatitude1);
        ...
        for (j = 1; j < points.length; j++) {
            if (points[j][1] > points[j - 1][1]) {
                dx1 = Utils.convertLngToPixel(points[j][0]);
                dy1 = Utils.convertLatToPixel(points[j][1]);
                dx2 = Utils.convertLngToPixel(points[j - 1][0]);
                dy2 = Utils.convertLatToPixel(points[j - 1][1]);
            } else {
                dx2 = Utils.convertLngToPixel(points[j][0]);
                dy2 = Utils.convertLatToPixel(points[j][1]);
                dx1 = Utils.convertLngToPixel(points[j - 1][0]);
                dy1 = Utils.convertLatToPixel(points[j - 1][1]);
            }
            x = (int) ((dx1 - longitude1) / divx);
            y = (int) ((dy1 - latitude1) / divy);
            xend = (int) ((dx2 - longitude1) / divx);
            ...
            if (y >= 0 && y < height && x >= 0 && x < width) {
                oy = (int) ((Utils.convertPixelToLat((int) (y * divy + latitude1)) - olatitude1) / odivy);
                ox = (int) ((Utils.convertPixelToLng((int) (x * divx + longitude1)) - olongitude1) / odivx);
                if (oy >= oheight) {
                    oy = oheight - 1;
                }
                if (ox >= owidth) {
