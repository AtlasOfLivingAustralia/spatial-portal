...
# determine the instance name from the instance path - remove path info with
# sed and then remove any trailing slash with tr
INSTANCE_NAME=`echo $TOMCAT_PATH | sed s/\.*instance_// | tr -d "/"`
...
if [ -e $NEW_WARFILE_DIR/$NEW_WARFILE ] ; then
	...
	$SSH_COMMAND  $SERVER "sudo /sbin/service tomcat stop $INSTANCE_NAME"
	...
	$SSH_COMMAND $SERVER "sudo /sbin/service tomcat start $INSTANCE_NAME"
