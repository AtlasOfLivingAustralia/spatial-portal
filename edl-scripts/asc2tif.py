...
for root, dirs, files in os.walk(edlconfig.dataset):
	for name in files:
		if ".asc" in name:
			...
			print(layername)		
			os.system(edlconfig.gdalapps+"/gdalwarp -s_srs '" + edlconfig.s_srs + "' -t_srs '" + edlconfig.t_srs + "' -srcnodata \"-9999\" -dstnodata \"-999\" -of GTiff " + os.path.join(root,name) + " "  + os.path.join(root,layername) + ".tif")
