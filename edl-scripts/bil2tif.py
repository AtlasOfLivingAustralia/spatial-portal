...
for root, dirs, files in os.walk(edlconfig.dataset):
	for name in files:
		if ".BIL" in name:
			...
			print(layername)		
			os.system(edlconfig.gdalapps+"/gdalwarp -ot Int16 -s_srs '" + edlconfig.s_srs + "' -t_srs '" + edlconfig.t_srs + "' -srcnodata \"0\" -dstnodata \"-9999\" -of GTiff " + os.path.join(root,name) + " "  + os.path.join(root,layername) + ".tif")
