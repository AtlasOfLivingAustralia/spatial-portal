...
function addLineDrawingLayer (layerName,serverUrl) {
...
function getpointInfo(e) {
    ...
    // to be made recursive to solve missed data points
            ...
            if ((! layer.isBaseLayer) && layer.queryable) {
                ...
                if (url) {
                    ...
                    if (is_ncWms(x)) {
                        ...
                        a.url = url;
                        queries[uuid] = a;
                    //OpenLayers.loadURL(url, '', a , setHTML2, setError);
                    }
...
function handleQueryStatus(theobj) {
    ...
    if (inaarray) {
        ...
        generalInfo = "<div id=\"featureinfoGeneral\">" + jQuery('#featureinfodepth').html() + "</div>";

        query_status = generalInfo + body;
            
        jQuery('#featureinfoheader').html(title).fadeIn(1200);
        jQuery('#featureinfostatus').html(query_status).fadeIn(400);
        return true;
    }
    else {
        return false;
...
// to be removed when getDepth is working
function getDepth_old(e) {


    var I= e.xy.x;
    var J= e.xy.y;
    var tolerance = 1; // pixel
    var bounds = new OpenLayers.Bounds();

    var topleft = map.getLonLatFromPixel(new OpenLayers.Pixel(I - 5,J - 5));
    var botright = map.getLonLatFromPixel(new OpenLayers.Pixel(I + 5,J +5));
    bounds.extend(topleft);
    bounds.extend(botright);

    /* not for showing on a map
    var layer = new OpenLayers.Layer.WFS( "World-Depths",
        " " + location.protocol + location.host + "/geoserver",
        {
            typename: "world_depths",
            maxfeatures: 1,
            REQUEST: "GetFeature",
            BBOX: bounds.toBBOX(),
            propertyName: 'depth'
    ...
    */
    ...
    var url = proxy_script +
    URLEncode("http://obsidian.bluenet.utas.edu.au/" +
        "geoserver/wfs?" +
        "request=GetFeature" +
        "&version=1.1.0" +
        "&typeName=topp:world_depths_single" +
        "&propertyName=single_depth" +
        "&BBOX=" + bounds.toBBOX() +
        ",EPSG:4326");
...
function mkTransectPopup(inf) {
    ...
    var html = "<div id=\"transectImageheader\">" +
                ...
                "<h3>" + inf.layerName + "</h3><h5>Data along the transect: </h5>" + inf.line +  "</div>" +
