/* Rangeslider.java
 *
 * This is a modified a copy of Slider.java
 */
 ...
 *  </ol>
 */
public class Rangeslider extends XulElement implements org.zkoss.zul.api.Slider {
	private String _orient = "horizontal";
	private int _curpos, _curmaxpos, _maxpos = 100, _pginc = -1;
	/** The name. */
	private String _name;
	private String _slidingtext = "{0}";

	static {
		addClientEvent(Rangeslider.class, Events.ON_SCROLL, CE_IMPORTANT | CE_DUPLICATE_IGNORE);
		addClientEvent(Rangeslider.class, Events.ON_SCROLLING, CE_DUPLICATE_IGNORE);
	}
	...
	throws WrongValueException {
		...
		else if (curpos > _curmaxpos) curpos = _curmaxpos;
	 ...
	 * {@link #getMaxpos} is assumed.
	 */
	public void setCurmaxpos(int curmaxpos)
	throws WrongValueException {
		if (curmaxpos < _curpos) curmaxpos = _curpos;
		else if (curmaxpos > _maxpos) curmaxpos = _maxpos;

		if (_curmaxpos!= curmaxpos) {
			_curmaxpos= curmaxpos;
			smartUpdate("curmaxpos", _curmaxpos);
		}
	}
	...
	throws IOException {
		...
		if(_curmaxpos != 0)
			renderer.render("curmaxpos", _curmaxpos);
		if(_maxpos != 100)
