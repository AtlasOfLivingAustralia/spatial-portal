...
public class Scatterplot {
    ...
    public Scatterplot(ScatterplotDTO scatterplotDTO, ScatterplotStyleDTO scatterplotStyleDTO, ScatterplotDataDTO scatterplotDataDTO) {
        ...
        if (layercount > 2) {
            ...
            htmlURL = AlaspatialProperties.getBaseOutputURL() + File.pathSeparator + scatterplotDTO.getId() + File.pathSeparator + "scatterplot.html";
            downloadURL = AlaspatialProperties.getBaseOutputURL() + File.pathSeparator + scatterplotDTO.getId() + File.pathSeparator + "scatterplot.zip";
        } else {
            //build just this one
    ...
    private void makeZip() {
        ...
        String pth = AlaspatialProperties.getBaseOutputDir() + uid + File.pathSeparator;

        Zipper.zipDirectory(pth, pth + "scatterplot.zip");
    }
    ...
    void buildScatterplot() {
        ...
        if (scatterplotDTO.getForegroundOccurrencesBs() != null) {
            resample(false);
            createDataset(false);
        }
    ...
    void renderScatterplot(String filename_part) {
        ...
        try {
            ...
            imageURL = AlaspatialProperties.getBaseOutputURL() + uid + File.pathSeparator + uid + filename_part + ".png";
    ...
    private double [][] samplePoints(double [][] p) {

        //update to make it faster when only 1 layer has changed;
        String [] layers = scatterplotDTO.getLayers();
        ...
        for(int i=0;i<layers.length;i++) {
            scanners[i] = new Scanner(sample.get(0));
        }
    ...
    public void reStyle(ScatterplotStyleDTO style) {
        scatterplotStyleDTO = style;
        buildScatterplot();
    ...
    private void makeHtml() {
        String pth = AlaspatialProperties.getBaseOutputDir() + scatterplotDTO.getId() + File.pathSeparator;
        String url = AlaspatialProperties.getAlaspatialUrl() + scatterplotDTO.getId() + File.pathSeparator;
