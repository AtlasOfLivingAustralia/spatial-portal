...
public class Pca {
	...
	public static int [][] getColours(double [][] cdata){
		...
		if(data[0].length <= 3){	//no need for pca
			...
			for(i=0;i<data.length;i++){
				for(j=0;j<nvars;j++){
					if(i == 0 || minval[j] > data[i][j]){
						minval[j] = data[i][j];
					}
					if(i == 0 || maxval[j] < data[i][j]){
						maxval[j] = data[i][j];
					}
				}
			}
			...
			for(i=0;i<data.length;i++){
				...
				for(;j<3;j++){						//continue
					//TODU: make missing value variable
    ...
    *     LANGUAGE:  FTN 5
    *     CALLED BY:
    *             CALL LRVT(N,PRECIS,EIGVAL,WORK,ASEVEC,IFAULT)
    *     PARAMETERS:
    *             PRECIS .....
    *             EIGVAL(N) .......
    *             WORK(N) .......
