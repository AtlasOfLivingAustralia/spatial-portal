...
public class Records {
    ...
    public Records(String filename,String wkt) throws IOException {
        System.out.println("Starting Records with file and wkt");
        int speciesEstimate = 250000;
        int recordsEstimate = 26000000;
        ...
        int start = 0;
        System.out.print("parsing wkt...");
        SimpleRegion areaWorking = SimpleShapeFile.parseWKT(wkt);
        System.out.println("done.");

        System.out.print("parsing records.csv...");
        BufferedReader br = new BufferedReader(new FileReader(filename));
        System.out.println("done");
        //CSVReader csv = new CSVReader(new FileReader(filename));
        ...
        line = new String[3];
        //while((line = csv.readNext()) != null) {
        System.out.println("Starting records iteration....");
        while ((rawline = br.readLine()) != null) {
            currentCount++;
            ...
            if (row == 0) {
                ...
                if (line.length >= 3) {
                    try {
                        double longitude = Double.parseDouble(line[header[1]]);
                        double latitude = Double.parseDouble(line[header[2]]);
                        System.out.println(longitude+","+latitude+","+areaWorking.isWithin(longitude, latitude));
