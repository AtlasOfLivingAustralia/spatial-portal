...
//        String lyr2value = jo.getString("name");        
        ...
        if (lsid instanceof BiocacheQuery) {
...

            {
                ...
                try {
                    ...
                    logger.error("error setting uploaded points", e);
                }
            }
        }
        ...
        for (int i = 0; i < layers.length - 1; i++) {
            for (int j = i + 1; j < layers.length; j++) {
                ...
                try {
                    ...
                    logger.debug("scatterplot image: " + layers[i] + "," + layers[j] + " > " + imageUrl);
                } catch (IOException ex) {
                    logger.debug("failed scatterplot image: " + layers[i] + "," + layers[j]);
                    ex.printStackTrace();
                }
        ...
        try {
            ...
            if (lsidQuery instanceof BiocacheQuery) {
                ...
                remoteLogger.logMapAnalysis(tToolName.getValue(), "Tool - Scatterplot", filterSa.getWkt(), lsidQuery.getQ(), sbenvsel, pid, extras, "SUCCESSFUL");
            } else {
