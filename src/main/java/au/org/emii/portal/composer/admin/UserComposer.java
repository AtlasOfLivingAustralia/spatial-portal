...
public class UserComposer extends GenericAutowireAutoforwardComposer {
    ...
    private PropertiesAuthenticationService propertiesAuthenticationService = null;
        ...
        if (user == null) {
            ...
            if (validate(true)) {
                ...
                if (! Validate.empty(passwordValue)) {
                    user.setPassword(
                        propertiesAuthenticationService.formatForProperties(
                            propertiesAuthenticationService.hashWithSalt(passwordValue)
                    ));
                }
    ...
    public void onClick$create() {
        ...
        if (validate(false)) {
            User user = new User(
                    usernameValue,
                    propertiesAuthenticationService.formatForProperties(propertiesAuthenticationService.hashWithSalt(passwordValue)));
            model.add(user);
    ...
    private void save() {
        ...
        if (propertiesAuthenticationService.updateProperties(properties, getPortalSession().getPortalUser().getUsername())) {
    ...
    public void afterCompose() {
        ...
        // make a convienience variable to avoid having to cast each time
        propertiesAuthenticationService = (PropertiesAuthenticationService) adminConsoleLoginService;
    ...
    private void makeModel() {
        ...
        Properties userProperties =  propertiesAuthenticationService.getUsers();
