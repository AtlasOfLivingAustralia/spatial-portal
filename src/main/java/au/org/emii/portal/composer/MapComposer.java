...
public class MapComposer extends GenericAutowireAutoforwardComposer {
    ...
    public void onClick$loadSavedMapButton() {
        ...
        String mapName = (String) savedMaps.getSelectedItem().getValue();
        ...
        logger.debug(String.valueOf(uMap.getbBox().getMaxLatitude()));
        logger.debug(String.valueOf(uMap.getbBox().getMaxLongitude()));
        logger.debug(String.valueOf(uMap.getbBox().getMinLatitude()));
        logger.debug(String.valueOf(uMap.getbBox().getMinLongitude()));



        for(MapLayer m : getUserDataManager().getActiveLayers(uMap.getId())) {
            try {
            logger.debug("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
            logger.debug(m.getMapLayerMetadata().getDefaultPalette());
            } catch (Exception ex) {
                logger.debug(ex.getMessage());
                logger.debug("stuffed it");
            }
        }
    ...
    public void onClick$mapSave() {
        ...
        if (createSavedMap.getText().length() < 1) {
            ...
            if (checkString(createSavedMap.getText())) {


                if (getUserDataManager().checkMapName(createSavedMap.getText())) {
                    getUserDataManager().saveMap(createSavedMap.getText());
                ...
                createSavedMapError.setValue("Only alpha numeric characters can be used");
                createSavedMapError.setVisible(true);
            }
        }
    ...
    public void loginActions() {
        ...
        updateUserMapList();
        toggleSavedSearchcont(true);


    }

    public void toggleSavedSearchcont(boolean show) {
        LeftMenuSearchComposer lmsc = getLeftmenuSearchComposer();
        lmsc.toggleSavedSearchcont(show);
