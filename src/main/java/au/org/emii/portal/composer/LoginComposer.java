...
public class LoginComposer extends UtilityComposer {
    ...
    /**
     * Kind - new user - zk autowired
     */
    private Listbox createKind = null;

    /**
     * Affiliation - new user - zk autowired
     */
    private Textbox createAffiliation = null;
    /**
    ...
    private void load() {
        ...
        createCountry.setModel(new CountryListModel(Config.getCountryCodes()));
    ...
    private void loginActions() {
        logger.info("FIXME: add your login actions here");
    }
    ...
    public void onClick$loginSubmitButton() {
        ...
        if (loginService.login(getPortalSession(), username, password)) {
            // login ok - close window
            logger.debug("login finished, login ok");
            getMapComposer().toggleLoginButton(false);
    ...
    public void onClick$registrationSubmitButton() {
        ...
        String country = createCountry.getValue();
        ...
        if (validateNewUser(lastName, firstName, email, address, state, zip, country, organisation, affiliation)) {
            ...
            switch (registrationService.register(lastName, firstName, email, address, state, zip, country, organisation, affiliation)) {
                case RegistrationService.REGISTRATION_FAIL_DUPLICATE:
                    logger.info("refusing to register duplicate user: " + email);
                    duplicateUserError.setVisible(true);
                    break;
                case RegistrationService.REGISTRATION_FAIL_INCOMPLETE:
                    ...
                    break;
                case RegistrationService.REGISTRATION_OK:
                    logger.debug("register new user ok: " + email);
                    registrationActions();
                    break;
                case RegistrationService.REGISTRATION_UNKNOWN_ERROR:
