...
public class LegendComposer extends UtilityComposer {
	...
	private final static int WINDOW_STACKING_BOUNDARY = 
		Config.getValueAsInt("window_stacking_boundary");
	
	private final static int WINDOW_STACKING_CONSTANT =
		Config.getValueAsInt("window_stacking_constant");
	...
	public String getLeftPosition() {	
		int menuWidth = Config.getValueAsPx("menu_default_width");
	...
	private int getStackingOffset() {
		...
		int offset;
		if (index > -1) {
			offset = (index * WINDOW_STACKING_BOUNDARY) % WINDOW_STACKING_CONSTANT;
		}
		else {
			offset = 0;
	...
	public String getTopPosition() {
		int north = Config.getValueAsPx("north_border_height");
		int offset = getStackingOffset();
