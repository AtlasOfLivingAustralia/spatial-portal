...
public class Config {
     ...
     */
    private static String PORTAL_PROPERTIES_FILE = "/portal.properties";

    /**
     * Fully qualified reference to PORTAL_PROPERTIES_FILE - used for IO
     */
    private static final URL PORTAL_PROPERTIES_URL = Config.class.getResource(PORTAL_PROPERTIES_FILE);
    ...
    private final static Properties portalProperties = new Properties();
     ...
     */
    private static ResourceBundle lang = ResourceBundle.getBundle("language");
    ...
    public static String getHostname() {
        if (hostname == null) {
            ...
            // values will be null on our very first call - from our custom Log4jLoader
            if (values != null) {
        ...
        private static String getPortalPropertiesFilename() {
            return (PORTAL_PROPERTIES_URL != null) ? PORTAL_PROPERTIES_URL.getFile() : null;
        }

    public static String getPortalNameStatic() {
        if (ReloadableProperties.guardUpdateProperties(getPortalPropertiesFilename(), lastReloaded, updating, portalProperties)) {
            ReloadableProperties.reloadProperties(portalProperties, updating, lastReloaded, ReloadableProperties.getInputStream(getPortalPropertiesFilename()));
        }

        return portalProperties.getProperty("portal_name");
    }

    /**
     * Save portal_name field in portal.properties and update static field
     * @param portalName
     */
    public static boolean updatePortalName(String portalName, String portalUsername) {
        boolean saved = false;
        ...
        try {
            props.store(
                    new FileOutputStream(
                        PORTAL_PROPERTIES_URL.getFile()),
                        getCompoundLang(
                            "properties_file_update_comment", new Object[] {portalUsername})
            );
            Config.portalName = portalName;
            saved = true;
        } catch (IOException ex) {
            Logger.getLogger(Config.class).error("error saving portal name", ex);
        }
        return saved;
    ...
    /**
     * Non-static accessor for portalName field - for use in EL
     * @return
     */
    public String getPortalName() {
        return getPortalNameStatic();
    }
